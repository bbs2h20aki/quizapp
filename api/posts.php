...
if ($method == 'GET') {
    ...
    if ($_POST != null && !$id) {
        extract($_POST);
        DB::query("INSERT INTO $tableName VALUES(null, :title, :body)", array(':title' => $title, ':body' => $body));
    ...
    if ($post != null) {
        if ($method == 'PUT') {
            ...
            DB::query("UPDATE $tableName SET title=:title, body=:body WHERE id = :id", array(':title' => $title, ':body' => $body, ':id' => $id));
            $data = DB::query("SELECT * FROM $tableName WHERE id=:id", array(':id' => $id));
